...
def one_hotify(labels):
  ...
  Arguments:
    labels: numpy array containing integer labels. The labels must be in range
    [0, num_labels - 1].
  ...
  """
  size = len(labels)
  one_hot_labels = np.zeros((size, np.max(labels) + 1))
...
def normalization(data_set, mean=None, std=None, eps=1e-6):
  ...
  eps: small constant to avoid division by very small numbers during
    normalization. If the a divisor is smaller than eps, no division will
    be carried out on that dimension.
  """
  if mean is None:
...
def global_contrast_normalization(data_set, eps=1e-6):
  ...
  Arguments:
    data_set: numpy array of shape (batch_size, dim). If the input has
      more than 2 dimensions (such as images), it will be flatten the data.
    eps: small constant to avoid division by very small numbers during
      normalization. If the a divisor is smaller than eps, no division will
      be carried out on that dimension.
  Returns:
